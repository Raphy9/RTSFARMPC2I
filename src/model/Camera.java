...
public class Camera {
    // Constantes de la taille de la camera (en nombre de tuiles a afficher)
    public static final int WIDTH = 22;
    public static final int HEIGHT = 13;

    // Position de la camera dans le monde du jeu
    private float x;
    private float y;

    public Camera() {
        this.x = (float) ((float)World.WIDTH/2 - (float) WIDTH/2+0.7); // Centrer la camera sur le monde
    ...
    /**
     * Deplace la camera en fonction des changements de position
     * Assure que la camera ne dépasse pas les limites du monde
    ...
    public void move(float deltaX, float deltaY) {
        if (this.x + deltaX >= 0 && this.x + deltaX < World.WIDTH - WIDTH) {
            this.x += deltaX; // Empeche de dépasser les bords gauche et droit
        }
        if (this.y + deltaY >= 0 && this.y + deltaY < World.HEIGHT - HEIGHT) {
            this.y += deltaY; // Empeche de dépasser les bords haut et bas
        }
    }
