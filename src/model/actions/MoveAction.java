 ...
 * Ce MoveAction est conçu pour etre exécuté par le thread du jardinier :
...
public class MoveAction extends Action {
     ...
     * @param targetX Les coordonnées x de la case cible.
     * @param targetY Les coordonnées y de la case cible.
     * @param onArrival Un Runnable qui sera exécuté une fois que le jardinier arrive sur la case cible. Peut etre null si aucun callback n'est nécessaire.
     */
     ...
     * Le déplacement en lui-meme est géré par le Gardener, donc ici on se concentre sur ce qui doit se passer a l'arrivée.
    ...
    public void perform(Gardener gardener, World world) {
        // Le déplacement en lui-meme (pathfinding + mouvement) est déjà
        ...
        // Exécuter le callback apres l'arrivée si fourni
        if (onArrival != null) {
            try {
                onArrival.run();
            } catch (Exception ex) {
